...
With your favorite text editor, add the following line to the `.zshrc` file on the `backseat` to add the compiler to path:

```zsh
export PATH=$PATH:$HOME/ardupilot/TARGET_DIR/gcc-arm-none-eabi-10-2020-q4-major/bin
```

Then, source the `.zshrc` file again and reactivate the `ardusub` virtual environment:
...
:::info
The first time you start the SITL, it will approximately 30 minutes to configure.
:::
