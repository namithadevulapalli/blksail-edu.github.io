...
Let a `Polygon` class be a class that has a `std::vector<Point>` data member and a `area` member function that returns the area of the polygon.
...
- `step` that takes a `double` parameter `dt` and moves the `AUV` using `speed`.
- `apply_accleration` that takes an `array<double>` of size 3 and accelerates the `AUV` by the given acceleration.
...
- `apply_angular_accleration` that takes a `double` parameter `angular_acceleration` and accelerates the `AUV` by the given angular acceleration.
